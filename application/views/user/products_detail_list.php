...
?>
 ...
 <title><?=$site['name']?> | <?=$user_site['name']?>产品管理</title>
</head>
    ...
    <section class="invoice no-margin">
      ...
      <div class="row">
          ...
          <div class="col-xs-12">
			...
			<tbody>
                            <?php foreach ($products_list as $article):?>
                            <?php
                            $id = $article['id'];
                            ?>
				<tr class="text-c">
                                                <td><?=$article['id']?></td>
                                                <td class="text-l"><u style="cursor:pointer" class="text-primary"><a href="<?=$site_url?>/user_products/products_edit/<?=$article['id']?>" title="查看"><?=$article['title']?></a></u></td>
                                                <td><?=$article['category']?></td>
                                                <td class="td-manage"><a style="text-decoration:none" class="ml-5" href="<?=$site_url?>/user_products/products_edit/<?=$id?>" title="编辑"><i class="glyphicon glyphicon-pencil"></i></a> 
                                                    <a style="text-decoration:none" class="ml-5" href="<?=$site_url?>/user_products/products_del/<?=$id?>" title="删除"><i class="glyphicon glyphicon-trash"></i></a></td>
				</tr>
                                <?php endforeach;?>
			</tbody>
		</table>

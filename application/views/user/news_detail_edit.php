...
<link rel="stylesheet" href="<?=$base_url?>static/plugins/select2/select2.min.css">
 <title><?=$site['name']?> | <?=$user_site['name']?>修改文章</title>
</head>
      ...
      <div class="row">
          ...
          <div class="col-xs-12">
            <article class="page-container">
                    <form class="form form-horizontal" id="form-article-edit" onsubmit="return news_edit_save()">
                            <div class="form-group">
                                <label for="title" class="col-sm-2 control-label">文章标题：</label>
                                    <div class="col-sm-10">
                                            <input type="text" class="form-control" value="<?=$news_detail['title']?>" placeholder="" id="title" name="title">
                                    </div>
                            </div>
                                ...
                                <label class="col-sm-2 control-label" for="category">分类栏目：</label>
                                    <div class="col-sm-10">
                                            ...
                                            <?php foreach ($category_list as $category):?>	
                                                <option value="<?=$category['id']?>" <?=$category['id']===$news_detail['category']?'selected="selected"':'';?>><?=$category['name']?></option>
                                            <?php endforeach;?>
                                            </select>
                                    </div>
                            </div>
                            ...
                            <div class="form-group">
                                    ...
                                    <div class="col-sm-10"> 
                                            <script id="content" name="content" type="text/plain" style="width:100%;height:400px;"><?=$news_detail['content']?></script> 
                                    </div>
                            </div>
                            <div class="form-group">
...
function news_edit_save()
    {
      ...
      }else
      {
        ...
        $.ajax({
        type:"POST",
        url:"<?=$site_url?>/user_news/news_edit_save",
        data:datastring,
        dataType:"json",

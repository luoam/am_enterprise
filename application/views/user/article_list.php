      ...
      <div class="row">
          ...
          <div class="col-xs-12">
			...
			<tbody>
				...
				<tr class="text-c">
                                                <td><?=$article['id']?></td>
                                                <td class="text-l"><u style="cursor:pointer" class="text-primary"><a href="<?=$site_url?>/admin_article/edit/<?=$article['id']?>" title="查看"><?=$article['title']?></a></u></td>
                                                <td><?=$article['category']?></td>
                                                <td><?=$article['source']?></td>
                                                <td><?=date("Y-m-d H:i",$article['dataline'])?></td>
                                                <td><?=$article['pageview']?></td>
                                                <td class="td-status"><span class="label label-success radius"><?=$article['pagedelete']?></span></td>
                                                <td class="td-manage"><a style="text-decoration:none" class="ml-5" href="<?=$site_url?>/admin_article/edit/<?=$id?>" title="编辑"><i class="glyphicon glyphicon-pencil"></i></a> 
                                                    <a style="text-decoration:none" class="ml-5" href="<?=$site_url?>/admin_article/del/<?=$id?>" title="删除"><i class="glyphicon glyphicon-trash"></i></a></td>
				</tr>
                                <?php endforeach;?>
			</tbody>
		</table>
